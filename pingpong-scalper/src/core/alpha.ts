...
export class MicroStatsCalculator {
  private priceHistory: number[] = [];
  private logReturns: number[] = [];

  calculateMicroStats(
    orderBook: OrderBookTick | null,
    priceHistory: number[],
    config: Config | null
  ): MicroStats | null {
  ...
  private updateLogReturns(priceHistory: number[]): void {
  ...
  private calculateVolatility(): number {
    if (this.logReturns.length < 2) return 0.001; // Дефолтная волатильность
  ...
  adaptParameters(
    currentStats: MicroStats,
    fillsPerMinute: number,
    consecutiveLosses: number,
    config: Config
  ): { s: number; orderNotional: number } {
