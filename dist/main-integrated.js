...
class IntegratedTradingSystem {
    ...
    async start() {
        ...
        try {
            ...
            // LLM анализ текущей ситуации при запуске
            if (this.config.llmEnabled) {
                await this.sendLLMStartupAnalysis();
            }
        }
        catch (error) {
        ...
        if (trade) {
            ...
            if (this.config.llmEnabled) {
                try {
                    ...
                    console.log('🧠 LLM анализ покупки завершен');
                    ...
                    console.error('❌ Ошибка LLM анализа покупки:', error);
                }
            }
        }
    }
    ...
    /**
     * LLM анализ при запуске системы
     */
    async sendLLMStartupAnalysis() {
        try {
            console.log('🧠 LLM анализирует текущую ситуацию...');
            // Получаем текущие данные по всем символам
            const currentAnalyses = [];
            for (const symbol of this.config.symbols) {
                const analysis = this.analyzer.getMarketAnalysis(symbol);
                if (analysis) {
                    currentAnalyses.push(analysis);
                }
            }
            if (currentAnalyses.length === 0) {
                console.log('⚠️ Нет данных для LLM анализа');
                return;
            }
            // Создаем сводку для LLM
            const marketSummary = {
                timestamp: Date.now(),
                symbols: currentAnalyses.map(a => ({
                    symbol: a.symbol,
                    price: a.currentPrice,
                    trend: a.trend,
                    trendStrength: a.trendStrength,
                    rsi: a.rsi,
                    isLocalMinimum: a.isLocalMinimum,
                    bollingerPosition: a.bollinger.position,
                    volumeRatio: a.volume.ratio,
                    support: a.support,
                    resistance: a.resistance
                })),
                totalTrades: this.paperTrading.getClosedTrades().length,
                openTrades: this.paperTrading.getOpenTrades().length,
                balance: this.paperTrading.getBalance(),
                winRate: this.paperTrading.getTradingStats().winRate
            };
            // Анализируем через LLM
            const llmAnalysis = await this.llmAnalyzer.analyzeMarketSituation(marketSummary);
            console.log('🧠 LLM анализ запуска завершен');
            // Отправляем в Telegram
            if (this.config.telegramEnabled) {
                await this.telegramNotifier.sendLLMCommentary('🚀 АНАЛИЗ ПРИ ЗАПУСКЕ СИСТЕМЫ', llmAnalysis.marketOverview, llmAnalysis.tradingOpportunities, llmAnalysis.riskAssessment, llmAnalysis.recommendations);
            }
        }
        catch (error) {
            console.error('❌ Ошибка LLM анализа запуска:', error);
        }
    }
    /**
...
//# sourceMappingURL=main-integrated.js.map